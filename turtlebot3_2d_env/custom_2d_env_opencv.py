...
class tbot_2d_env(Env):
	...
	def __init__(self):
		...
		self.ep_duration = 300
		self.obstacles = [np.array([1.5, 1.5, 0.14]), np.array([2.5, 1.5, 0.14]), np.array([2.5, 2.5, 0.14]), np.array([1.5, 2.5, 0.14])]
		self.img = np.ones((400,400,3), dtype=np.uint8)*255

	# Function to randomize start position of the robot
	def random_start(self):
		self.state[12] = np.random.random()*0.2 + 0.3
		self.state[13] = np.random.random()*0.2 + 0.3
		self.state[14] = 0

	# Function to randomize goal position of the robot
	def random_goal(self):
		self.state[15] = np.random.random()*0.2 + 3.1
		self.state[16] = np.random.random()*0.2 + 3.1
		self.state[17] = 0

	# Function to start robot at a fixed position
	def fixed_start(self):
		self.state[12] = 0.6
		self.state[13] = 0.6
		self.state[14] = 0

	# Function to spawn the goal of robot at a fixed position
	def fixed_goal(self):
		self.state[15] = 3.3
		self.state[16] = 3.3
		self.state[17] = 0

	# Function to reset the environment to spawn the robot at a new random start location, new goal position, and reset the observation space
	def reset(self):
		self.state = np.array([1.5,1.5,1.5,1.5,1.5,1.5,1.5,1.5,1.5,1.5,1.5,1.5, 0.3, 0.3, 0, 3.3, 3.3, 0, 0, 0])
		self.random_start()
		self.random_goal()
	...
	# The main step function of the environment that returns the observation space, rewards, done flag and info (info is not used)
	def step(self, action):
		...
		for ray_scan in self.state[0:12]:
			if ray_scan < 0.20:
				...
				reward += ray_safe_factor*(ray_scan - 1.5)/(r_i + r_s)
	...
	# Function to read the lidar data from the environment
